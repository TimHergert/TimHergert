namespace farmsimulation {
    export let animalpath: Path2D[];
    export let cowPath: Path2D;

    export let animals: number[][][] = [
        [
    ...
    export function createPaths(): void {
        animalpath= createAsteroidPaths(animals);
        cowPathPath = createUfoPath();
    }

    function createAnimalPaths(_shapes: number[][][]): Path2D[] {
        let paths: Path2D[] = [];
